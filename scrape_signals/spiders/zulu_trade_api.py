...
class ZuluTradeSpiderAPI(BaseCrawlSignalSpider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.start_urls = [Constant.ZULU_API_URL_TEMPLATE.format(external_trader_id=self.external_trader_id)]
    ...
    def parse(self, response, kwargs=None):
        ...
        trader_item['external_trader_id'] = self.external_trader_id
        ...
        if trader_item['hash'] != self.hash_dict["previous_hash"]:
            self.logger.info(
                f'{self.external_trader_id} new hash {trader_item["hash"]}, {self.hash_dict["previous_hash"]}')
            self.hash_dict['previous_hash'] = trader_item['hash']
            yield trader_item

        else:
            self.logger.info(f'{self.external_trader_id} old hash {trader_item["hash"]}, Nothing to update')
            yield None
